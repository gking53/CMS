...
int	InitializePlayback(void)
{
	...
	if (rc) {
		qDebug("Unable to open Output sound Device! Error %x.", rc);
		return(0);
	}
	return(1);
}
...
int PlayBuffer(short *piBuf, long lSamples)
{
	...
	if (rc) {
		qDebug("Failed preparing WAVEHDR, error 0x%x.", rc);
		return(0);
	}
...
int InitializeRecording(void)
{
	...
	if (rc) {
		qDebug("Unable to open Input sound Device! Error %x.", rc);
		return(0);
	}
	...
	if (rc) {
		qDebug("Failed preparing input WAVEHDR, error 0x%x.", rc);
		return(0);
	}
