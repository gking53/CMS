...
int main() {
	...
	char msgOut[] = "Hi there N O R M I E";						// Sent message
	char msgIn[BUFSIZE] = { NULL };							// Received message
	initPort();										// Initialize the port
	//outputToPort(msgOut, strlen(msgOut)+1);			// Send string to port - include space for '\0' termination
	//Sleep(1000);		
	while (1) {// Allow time for signal propagation on cable 
...
void inputFromPort(LPVOID buf, DWORD szBuf) {
	...
	printf("Waiting for com event\n");
	while(1) {
		if (WaitCommEvent(hCom, &lpEvents, NULL)) {
			if (ReadFile(hCom, buf, szBuf, &NumberofBytesRead, NULL)) {
...
void createPortFile() {
	...
	hCom = CreateFile(
		"COM4",										// COM port number  --> If COM# is larger than 9 then use the following syntax--> "\\\\.\\COM10"
